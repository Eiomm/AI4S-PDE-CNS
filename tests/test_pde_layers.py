...
def test_task1_spec_defaults_to_competition_paths():
    ...
    assert DEFAULT_TASK1_FNO_WEIGHTS == {
        "nu0.001": 0.01,
        "nu0.01": 0.31,
        "nu0.1": 0.66,
        "nu1.0": 0.02,
...
def test_task1_workflow_runs_validation_and_submission(tmp_path):
    ...
    val_result = workflow.run_validation({"nu0.1": 1.0}, run_name="val")
    submit_result = workflow.run_test_submission({"nu0.1": 1.0}, run_name="submission", train_time=12.5)
...
def test_task1_workflow_accepts_checkpoint_path_overrides(tmp_path):
    ...
    workflow = Task1FNOWorkflow(
        ...
        code_dir=code_dir,
        methodology_path=methodology_path,
        checkpoint_paths={"nu0.1": custom_checkpoint},
        project_root=tmp_path,
    )

    command = workflow._fno_ensemble_command(spec.test_input_path, {"nu0.1": 1.0}, tmp_path / "pred.hdf5")

    assert str(custom_checkpoint) in command


def test_parse_checkpoint_overrides_maps_key_value_pairs(tmp_path):
    path = tmp_path / "best.pt"

    overrides = parse_checkpoint_overrides([f"nu0.1={path}"])

    assert overrides == {"nu0.1": path}

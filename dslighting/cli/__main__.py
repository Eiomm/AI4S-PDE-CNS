    ...
    workflow_metadata = {
        ...
        "react": {
            ...
            "use_cases": [
                ...
                "Protocol-constrained tasks",
            ],
            "default_model": "gpt-4o",
            "parameters": {"temperature": 0.7},
            "unique_params": {
                "agent_runtime.max_steps": "Maximum agent turns (default: 10)",
                "agent_runtime.observation.max_tokens": "Observation token budget (default: 4000)",
                "agent_runtime.observation.head_tokens": "Observation head tokens kept after truncation (default: 2000)",
                "agent_runtime.observation.tail_tokens": "Observation tail tokens kept after truncation (default: 2000)",
                "agent_runtime.context.keep_recent_turns": "Raw recent turn window kept by the context manager (default: 14)",
                "agent_runtime.context.strategy": "Context strategy: recent_turns | summarize_old_turns | hybrid (default: hybrid)",
                "output_contract.require_output_before_completion": "Require expected output artifact before final completion",
            },
            "class": presets.ReAct,
        },
    }
...
def build_cli_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="dslighting", description="DSLighting - Data Science Agent CLI"
    )
    ...
    # version command
    version_parser = subparsers.add_parser("version", help="Show DSLighting version information")
    version_parser.set_defaults(func=cmd_version)

    # help command
    help_parser = subparsers.add_parser("help", help="Show DSLighting help and quick start guide")
    help_parser.set_defaults(func=cmd_help)

    # workflows command
    workflows_parser = subparsers.add_parser(
        "workflows", help="List all available workflows with details"
    )
    workflows_parser.set_defaults(func=cmd_workflows)

    # example command
    example_parser = subparsers.add_parser("example", help="Show workflow example code")
    example_parser.add_argument(
        "workflow", help="Workflow name (e.g., aide, autokaggle, data_interpreter, etc.)"
    )
    example_parser.set_defaults(func=cmd_example)

    # quickstart command
    quickstart_parser = subparsers.add_parser("quickstart", help="Show detailed quick start guide")
    quickstart_parser.set_defaults(func=cmd_quickstart)

    # detect-packages command
    detect_parser = subparsers.add_parser(
        "detect-packages", help="Detect and save available Python packages to config"
    )
    detect_parser.add_argument("-c", "--config", help="Path to config.yaml file", default=None)
    ...
    show_parser = subparsers.add_parser("show-packages", help="Show detected packages from config")
    show_parser.add_argument("-c", "--config", help="Path to config.yaml file", default=None)
    show_parser.set_defaults(func=cmd_show_packages)

    # validate-config command
    validate_parser = subparsers.add_parser(
        "validate-config", help="Validate DSLighting configuration"
    )
    validate_parser.add_argument("-c", "--config", help="Path to config.yaml file", default=None)

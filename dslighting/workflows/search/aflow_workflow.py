...
class AFlowWorkflow:
    ...
    async def optimize(self) -> str:
        ...
        # DABench tasks do not consistently provide prepare_val functions. AFlow
        # already evaluates DABench candidates against prepared validation/test
        # directories directly, so avoid mutating the shared benchmark registry
        # into validation mode for concurrent task runs.
        all_dabench = all(
            str(problem.get("competition_id") or "").startswith("dabench-")
            for problem in self.benchmark.problems
        )
        if all_dabench:
            logger.info("DABench detected; keeping benchmark in test mode for AFlow optimization.")
        else:

"""
Experience state for meta-optimization (AFlow-style evolutionary search).
"""
import json
import logging
...
class Experience(State):
    ...
    def created_at(self) -> datetime:
        """When this experience store was created."""
        return self._created_at

    def get(self, key: str, default: Optional[Any] = None) -> Optional[Any]:
        """Get a value from the experience JSON store."""
        try:
    ...
    def set(self, key: str, value: Any) -> None:
        """Set a value in the experience JSON store."""
        try:
            with open(self.experience_file, "r", encoding="utf-8") as f:
                data = json.load(f)
        except (FileNotFoundError, json.JSONDecodeError):
    ...
    def delete(self, key: str) -> bool:
        """Delete a value from the experience JSON store."""
        try:
            with open(self.experience_file, "r", encoding="utf-8") as f:
                data = json.load(f)
        except (FileNotFoundError, json.JSONDecodeError):
    ...
    def clear(self) -> None:
        """Clear all persisted optimizer state."""
        with open(self.scores_file, "w", encoding="utf-8"):
            pass
        with open(self.experience_file, "w", encoding="utf-8") as f:
            json.dump({}, f)

    def snapshot(self) -> bytes:
        """Create a checkpoint snapshot of the optimizer state files."""
        payload = {
            "scores": self.scores_file.read_text(encoding="utf-8")
            if self.scores_file.exists()
            else "",
            "experience": json.loads(self.experience_file.read_text(encoding="utf-8"))
    ...
    def restore(self, data: bytes) -> bool:
        ...
        try:
            ...
            self.experience_file.write_text(
                json.dumps(experience, indent=4, default=str),
                encoding="utf-8",
            )

...
_FENCED_JSON_RE = re.compile(
    # ``` followed optionally by a json/JSON language tag (NOT other languages),
    # then a newline, the body, an optional trailing newline, and closing ```.
    r"```(?:json|JSON)?\s*\n(?P<body>.*?)\n?```",
    re.DOTALL,
)
...
def _strip_markdown_fences(content: str) -> str:
    """Normalize JSON responses from providers (notably Claude through
    OpenAI-compatible proxies) that wrap JSON in ```json ... ``` fences.

    Strategy:
      1. If the text contains a ```json ... ``` (or plain ``` ... ```) block,
         return the first such block's body.
      2. Otherwise return the input stripped, unchanged.

    Only `json` and untagged fences are stripped. Other language tags such
    as ```python ... ``` are left alone so the downstream JSON parser still
    raises a clear error (we should not silently accept non-JSON content
    just because it was inside a code fence).
    """
...
class LLMCallExecutor:
    ...
    async def execute(self, spec: LLMCallSpec) -> Any:
        ...
        for semantic_attempt in range(1, validation_attempts + 1):
            ...
            response_content = _strip_markdown_fences(response_content)
            try:
